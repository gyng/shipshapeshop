  ...
  return (
    <div className="orrery-panel orrery-bottom">
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-2)' }}>
        <span style={{ fontSize: 'var(--fs-eyebrow)', color: 'var(--c-text-dim)', whiteSpace: 'nowrap' }}>{tr('orrery.timeline')} · {L}</span>
...
function OrbitDetail({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  return (
    <div style={{ borderTop: '1px solid var(--c-border)', marginTop: 'var(--sp-1_5)', paddingTop: 6, display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      <div style={{ fontSize: 'var(--fs-micro)', color: 'var(--c-text-faint)' }}>{tr('orrery.period')} {orb.period} · {tr('orrery.anchor')} {orb.anchor[0]},{orb.anchor[1]}{orb.tuned ? ' · ✎' : ''}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <span style={{ fontSize: 'var(--fs-caption)', minWidth: 16, textAlign: 'center', fontVariantNumeric: 'tabular-nums' }}>{orb.phase}</span>
        <button style={mini} onClick={() => stepPhase(1)}>+</button>
      </div>
  ...
  return (
    <div className="orrery-wrap">
      ...
      <div className="orrery-panel orrery-left">
        ...
        {library.map((s) => (
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-1_5)', fontSize: 'var(--fs-caption)', padding: 'var(--sp-1) var(--sp-1_5)', borderRadius: 'var(--r-md)', border: '1px solid var(--c-border)', background: 'var(--c-surface-2)', color: 'var(--c-text-secondary)', cursor: canDeploy ? 'pointer' : 'not-allowed', opacity: canDeploy ? 1 : 0.5, textAlign: 'left' }}
          >
      ...
      <div className="orrery-panel orrery-right">
        <div style={{ fontSize: 'var(--fs-eyebrow)', fontWeight: 'var(--fw-heavy)', color: 'var(--c-text-dim)', textTransform: 'uppercase', letterSpacing: 0.5 }}>{tr('orrery.active')} · {loadout.length}</div>
        <div style={{ fontSize: 'var(--fs-micro)', color: 'var(--c-text-faint)' }}>{tr('orrery.budget')} {view.euler_used}/{view.euler_cap}</div>
        {loadout.map((id) => {
          const s = shapes[id]
          if (!s) return null
          return (
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-1_5)', fontSize: 'var(--fs-caption)', padding: 'var(--sp-1) var(--sp-1_5)', borderRadius: 'var(--r-md)', border: `1px solid ${sel === id ? RARITY_COLOR[s.rarity] : 'var(--c-border)'}`, background: sel === id ? `${RARITY_COLOR[s.rarity]}1c` : 'var(--c-surface-2)', color: 'var(--c-text-secondary)', cursor: 'pointer', textAlign: 'left' }}
            >
              <span style={{ fontSize: 'var(--fs-h4)' }}>{glyphOf(s.family)}</span>
              <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.nick}</span>
            </button>
