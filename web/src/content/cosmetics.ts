...
export interface GemFinishSpec {
  ...
  mat: {
    ...
    metallic?: number // 0 = dielectric glass; 1 = a colored metal MIRROR (F0 = body colour, no transmission) — real metal
    retro?: number // 0 = normal; >0 = RETROREFLECTIVE — returns light back toward its source (road-sign / cat's-eye look)
    ripple?: number // DYNAMIC: 0 = still; >0 = an animated water / heat-haze surface (the normal shimmers with scrolling noise)
    fire?: number // DYNAMIC: 0 = none; >0 = a live fire surface (scrolling emissive flame)
  }
...
export const GEM_FINISHES: GemFinishSpec[] = [
  ...
  { id: 206, name: 'Liquid Chrome', cost: 11000, desc: 'A mirror-bright puddle of molten metal that drinks the light and pours it right back.', mat: { colorTint: '#dfe4ec', metallic: 1, transmissionMul: 0.22, roughness: 0.05, clearcoat: 1, clearcoatRoughness: 0.03, envMapIntensityMul: 1.5, iridescence: 0 }, swatch: 'linear-gradient(135deg, #f2f4f8, #aab0bd, #6f7686, #d7dbe2)', hues: ['#f2f4f8', '#aab0bd', '#6f7686'] },
  ...
  { id: 241, name: 'Retroreflector', cost: 2600, desc: 'A skin of tiny corner mirrors — it throws every ray straight back the way it came, so the whole shape lights up toward you from any angle, like a road sign in headlights.', mat: { colorTint: '#eef2ff', retro: 0.92, roughness: 0.06, transmissionMul: 0.2, envMapIntensityMul: 1.3 }, swatch: 'radial-gradient(circle at 50% 45%, #ffffff, #c6d0ea 55%, #8a94b4)', hues: ['#ffffff', '#dfe6ff', '#b6c2e6'] },
  { id: 242, name: "Cat's Eye", cost: 2800, desc: 'A retroreflective amber bead-coat — it glows back at whoever looks, a warm animal shine in the dark.', mat: { colorTint: '#ffcf7a', retro: 0.9, roughness: 0.08, transmissionMul: 0.25, emissiveIntensity: 0.12, envMapIntensityMul: 1.2 }, swatch: 'radial-gradient(circle at 50% 45%, #fff2d0, #ffb84a 55%, #8a5a12)', hues: ['#fff2d0', '#ffcf7a', '#e09a3a'] },
  // ── Dynamic finishes ✦ — time-animated surfaces that flow LIVE (they render every frame; no freeze) ──
  { id: 243, name: 'Water', cost: 2400, desc: 'Clear as a mountain spring and never quite still — the surface ripples and shivers as if a breeze just crossed it.', mat: { colorTint: '#d0eeff', ripple: 0.55, iorAdd: -0.1, transmissionMul: 1.05, roughness: 0.02 }, swatch: 'linear-gradient(135deg, #eaffff, #a6e0ff, #5fb0ff)', hues: ['#eaffff', '#a6e0ff', '#5fb0ff'] },
  { id: 244, name: 'Wildfire', cost: 3200, desc: 'A shape wrapped in living flame — orange and gold licking upward, never the same from one glance to the next.', mat: { colorTint: '#2a0e05', fire: 1.0, transmissionMul: 0.3, roughness: 0.4, iorAdd: 0.05 }, swatch: 'radial-gradient(circle at 50% 62%, #fff0b0, #ff8a1a 45%, #b02808 75%, #1a0604)', hues: ['#fff0b0', '#ff8a1a', '#ff4a10'] },
